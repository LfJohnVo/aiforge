...
- Módulos Python: **85**
- Líneas de código: **11104**
- Clases: **129** · Funciones y métodos de nivel superior: **359**
...
| [`runtime.py`](../src/agent_forge/runtime.py) | Composition root: turns a profile plus an environment into a running cell. | 273 | `installed_capabilities`, `Settings`, `Runtime`, `check_capabilities`, `load_model_policy`, `build_runtime` |
...
| [`admin.py`](../src/agent_forge/api/admin.py) | Administration and operation API. | 306 | `ApprovalDecision`, `require_authenticated`, `effective_config`, `list_approvals`, `decide_approval`, `task_status` (+5) |
...
| [`graph.py`](../src/agent_forge/core/graph.py) | The agentic graph. | 684 | `GateOutcome`, `identity_gate`, `GraphDeps`, `_digest`, `_intake`, `_governance_gate` (+16) |
...
| [`test_memory_redis.py`](../tests/integration/test_memory_redis.py) | The memory contract against a real Redis. | 151 | `redis_url`, `store`, `test_round_trip_and_delete`, `test_ttl_is_applied`, `test_scan_is_scoped_to_the_pattern`, `test_health_is_true_against_a_live_server` (+4) |
...
| [`test_api.py`](../tests/unit/test_api.py) | The HTTP surface: auth, the OpenAI-compatible channel, admin and health. | 381 | `_env`, `_runtime`, `runtime`, `app`, `_static_lifespan`, `client` (+17) |
